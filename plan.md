...
> **현황:** 단일 `index.html` 프로토타입 (v9.3 — PM Agent 4차 패치 적용)
...
| "독촉 메일 발송" 버튼 | ✅ | v9.3: 동적 렌더링 + sendNudge() 연결 (오프라인 시 UI만) |
| 리스크 현황 테이블 (상태별 필터) | ⚠️ | RISKS 배열 하드코딩 |
| 리스크 상세 모달 (테이블 → 클릭) | ✅ | v9.2 추가: 영향도/가능성/설명/개선과제/제출내용/평가의견 표시 |
| 연도/월 셀렉터 | ✅ | v9.3: setYear()/setMonth() 연동, 기본값 동기화 완료 |
...
| 연도 셀렉터 (2024~2026) | ✅ | v9.3: setYear()/setMonth() + 기본값 동기화 |
...
#### 2026-03-16 4차 적용 (index.html v9.2 → v9.3)

PM Agent 4차 분석 기반 일괄 패치. 프로토타입 완성도 대폭 향상 (17개 항목).

| # | 수정 내용 | 분류 |
|---|----------|------|
| 54 | 월 셀렉터 완전 수정 — PC/모바일 `onchange="setMonth()"` + `setMonth()` 함수 신규 | 🔴 기능 |
| 55 | 독촉 메일 하드코딩 제거 → `<div id="dash-missing-alerts">` 동적 렌더링 교체 | 🔴 기능 |
| 56 | XSS 방어 — `_esc()` 헬퍼 + 리스크 테이블 렌더 적용 | 🔴 보안 |
| 57 | 차트 탭 전환 시 재렌더 — `navigateTo()` 패치 (dash/teams/myteam) | 🔴 안정성 |
| 58 | 셀렉터 기본값 동기화 — `_syncSelectors()` 로그인 시 curYear/curMonth 반영 | 데이터 |
| 59 | 보고서 연도 셀렉터 동적 기본값 — curYear 자동 설정 | UX |
| 60 | 하드코딩 텍스트 동적화 — "2025년 3월 기준" → `${curYear}년 ${curMonth}월 기준` | UX |
| 61 | 리스크 중복 감지 — 같은 title+team 존재 시 경고 | 검증 |
| 62 | 리스크명 길이 검증 — 추가/수정 시 4자 이상, 200자 이하 | 검증 |
| 63 | 이행 내용 textarea maxlength 2000자 + 글자수 카운터 | 검증 |
| 64 | Excel 더블클릭 방어 — confirmExcelImport 중복 실행 차단 | 안전 |
| 65 | Dirty form eval/master 확장 — 편집 중 이탈 시 경고 | 안전 |
| 66 | callFlow 15초 AbortController 타임아웃 | 안정성 |
| 67 | navigator.onLine — 온/오프라인 전환 시 toast 알림 | UX |
| 68 | 리사이즈 디바운스 200ms + myteam 차트 포함 | 성능 |
| 69 | TREND 빈 데이터 null 처리 — 가짜 70% 제거, 차트 null skip | 데이터 |
| 70 | submitStatus/status 동기화 — 오프라인 로드 시 자동 통합 | 데이터 |
...
✅ [D2-1] 주요 화면 Empty State — v9.2에서 해결
