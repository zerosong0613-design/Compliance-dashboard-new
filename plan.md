...
> **작성일:** 2026-03-18 (최종 업데이트: 2026-03-23)
> **현황:** 단일 `index.html` (v10.3, ~6,900줄) — 단계별미션 서브페이지 분리, monkey-patch 6개
...
| 기능 | 상태 | 비고 |
|------|------|------|
| PC 사이드바 (접기/펼치기) | ✅ | 단계별미션 하위 서브메뉴 지원 (v10.3) |
| 모바일 하단 탭 (Bottom Nav) | ✅ | |
| 모바일 드로어 (햄버거 메뉴) | ✅ | |
...
| 단계별 미션 랜딩 (Phase Control) | ✅ | 온보딩 인트로 + 신호등 아코디언 카드 (v10.3 리디자인) |
| 단계별 미션 서브페이지 (phase1~5) | ✅ | 빵크럼 + 히어로 + 할일카드 + 체크리스트 + 이전/다음 네비 (v10.3) |
| 사이드바 서브메뉴 (5단계 펼침/닫힘) | ✅ | 완료 단계 녹색 dot + 토글 접기 (v10.3) |
| 순차 진행 제한 | ✅ | 이전 단계 미완료 시 완료 확정 잠금 (미리보기 허용, v10.3) |
| 연간보고서 수동 체크 | ✅ | 5단계 "연간 보고서 생성" 자동→수동 클릭 토글 (v10.3) |
...
│   │   ├── reports.js          ← 보고서 생성
│   │   ├── phasecontrol.js     ← 단계별미션 랜딩 + 서브페이지 (v10.3 신설)
...
> **분석일:** 2026-03-15 (최종 업데이트: 2026-03-23)
> **방법:** PM Agent(제품 관리 관점) + Designer Agent(UX/UI 관점) 교차 분석
> **대상:** index.html(v8→v10.3), plan.md, SETUP.md, architecture.svg
...
| D3-3 | 온보딩 가이드 (Tooltip Tour) | ⚠️ 부분해결 | 단계별미션 랜딩이 온보딩 역할 (v10.3), 풀투어는 미구현 |
...
#### 2026-03-23 6차 적용 (index.html v10.2 → v10.3)

단계별미션 랜딩+서브페이지 분리 + 사이드바 서브메뉴 + 순차 진행 제한.

| # | 파일 | 수정 내용 | 분류 |
|---|------|----------|------|
| 113 | index.html | 단계별미션 랜딩 리디자인: 온보딩 인트로 + 신호등 아코디언 카드 | UX |
| 114 | index.html | phase1~phase5 서브페이지 신설: 빵크럼 + 히어로 + 할일카드 + 체크리스트 + 이전/다음 네비 | 기능 |
| 115 | index.html | 사이드바 서브메뉴: 단계별미션 클릭 시 5개 하위메뉴 펼침/닫힘 | UX |
| 116 | index.html | 사이드바 서브메뉴 완료 상태: 녹색 dot + 취소선 (phase 확정 시 자동 반영) | UX |
| 117 | index.html | 모바일 드로어에도 동일한 서브메뉴 적용 | UX |
| 118 | index.html | 순차 진행 제한: 이전 단계 미완료 시 완료 확정 잠금 (미리보기 허용) | 기능 |
| 119 | index.html | `_isPrevPhaseCompleted()` 헬퍼 + `confirmPhaseComplete()` 이전단계 검증 추가 | 기능 |
| 120 | index.html | 서브페이지/대시보드 phase-hero: 이전 단계 미완료 시 잠금 메시지 표시 | UX |
| 121 | index.html | 5단계 "연간 보고서 생성" 체크: 자동 감지 → 수동 클릭 토글로 변경 | 기능 |
| 122 | index.html | `_toggleManualCheck()` 함수 + 체크리스트 3곳 수동 체크 UI 적용 | 기능 |
| 123 | index.html | PPT 생성 시 자동 reportGenerated 훅 제거 | 정리 |
| 124 | index.html | 아코디언 체크 클릭 시 카드 닫힘 버그 수정 (열린 카드 상태 복원) | 버그 |
| 125 | index.html | 신호등 CSS: `.phase-signal` sig-green/sig-blue/sig-amber/sig-gray + 맥동 애니메이션 | CSS |
| 126 | index.html | 완료 단계 카드: 녹색 하단 바 + 제목 취소선 + 투명도 변경 | CSS |
| 127 | index.html | PAGE_TITLES/HIDE_TOPBAR_RIGHT에 phase1~5 추가 | 기능 |
| 128 | index.html | `navigateTo()` phase1~5 라우팅 + 사이드바 하이라이트 phasecontrol 유지 | 기능 |
| 129 | plan.md | v10.3 기능 인벤토리 + 적용 이력 업데이트 | 문서 |
...
☑ [NEW] 단계별미션 랜딩 리디자인 — 온보딩 인트로 + 신호등 아코디언 (v10.3)
☑ [NEW] phase1~5 서브페이지 — 빵크럼 + 히어로 + 체크리스트 + 이전/다음 네비 (v10.3)
☑ [NEW] 사이드바 서브메뉴 — 5개 하위 메뉴 펼침/닫힘 + 완료 녹색 dot (v10.3)
☑ [NEW] 순차 진행 제한 — 이전 단계 미완료 시 완료 확정 잠금 (v10.3)
☑ [NEW] 연간보고서 수동 체크 — 자동→클릭 토글 전환 (v10.3)
☑ [FIX] 아코디언 체크 클릭 시 카드 닫힘 버그 수정 (v10.3)
```
