...
> **현황:** 단일 `index.html` 프로토타입 (v9.4 — PPT 수정 + 샘플데이터 + 차트 개선)
...
| 기능 | 상태 | 비고 |
|------|------|------|
| KPI 카드 4종 (관리 리스크 / 제출완료 / 달성율 / 평가대기) | ✅ | v9.3: 실데이터 기반 계산 (오프라인 시 월별 동적 샘플) |
| KPI 카운터 애니메이션 | ✅ | |
| 팀별 이행 달성율 바 차트 | ✅ | v9.4: 월별 동적 샘플 + TEAMS_DATA 자동 계산 |
| 월별 달성율 추이 꺾은선 차트 (Canvas) | ✅ | v9.4: Y축 동적 범위 + 연초 경계선 표시 |
| 리스크 히트맵 (영향도×가능성) | ✅ | 정적 렌더링 (실데이터 연동 필요) |
| 미제출 현황 알림 카드 | ✅ | v9.3: 동적 렌더링 (RISKS 기반 팀·건수 자동 계산) |
...
| 연간 PPT 미리보기 | ✅ | |
| PPT 내 실데이터 반영 | ✅ | v9.4: KPI/팀/리스크 실데이터 연동, 16:9 레이아웃 재설계 |
...
#### 2026-03-16 5차 적용 (index.html v9.3 → v9.4)

PPT 생성 버그 수정 + 샘플 데이터 동적화 + 차트 개선.

| # | 수정 내용 | 분류 |
|---|----------|------|
| 71 | navigateTo 4중 체인 래핑 → 단일 통합 패치 (로그인 화면 클릭 불가 버그) | 🔴 버그 |
| 72 | dday 변수 중복 선언 → 전체 스크립트 로드 실패 버그 | 🔴 버그 |
| 73 | 월간 PPT: 월 표시 숫자→텍스트 변환 ("3" → "3월") | PPT |
| 74 | 월간/연간 PPT: 16:9 비율 명시적 설정 (defineLayout 13.33×7.5) | PPT |
| 75 | 월간 PPT: KPI 객체 구조 전환 + 7.5인치 높이 균등 분배 | PPT |
| 76 | 연간 PPT: KPI 색상코드 텍스트 출력 버그 수정 + 전면 재작성 | PPT |
| 77 | 연간 PPT: getAnnualData teams 필드명 통일 (team→name) | PPT |
| 78 | 연간 PPT: 미리보기↔실제 파일 레이아웃 통일 | PPT |
| 79 | 오프라인 샘플 데이터 연간 사이클 재설계 (1월 낮음→12월 높음) | 데이터 |
| 80 | TREND 동적 생성: 팀별 역량 차이 + 월 진행 상승 + 연초 급락 | 데이터 |
| 81 | drawChart Y축 동적 범위 (40% 고정 → 데이터 기반 자동 계산) | 차트 |
| 82 | drawTeamsChart Y축 동적 범위 (동일 적용) | 차트 |
| 83 | 차트 연초 경계선 표시 (1월 위치에 주황 점선 + "▼ 연초" 라벨) | 차트 |
